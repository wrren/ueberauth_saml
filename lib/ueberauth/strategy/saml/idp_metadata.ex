defmodule SAML.IDPMetadata do
  ...
  """
  def decode(xml_string) when is_binary(xml_string), do: SweetXml.parse(xml_string, namespace_conformant: true) |> decode
  def decode(xml) do
    xml   
    |> xpath(
        ~x"/md:EntityDescriptor",
        entity_id: ~x"./@entityID"s,
        login_location: ~x"./md:IDPSSODescriptor/md:SingleSignOnService[@Binding='urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST']/@Location"s,
        logout_location: ~x"./md:IDPSSODescriptor/md:SingleLogoutService[@Binding='urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST']/@Location"s,
        name_format: ~x"./md:IDPSSODescriptor/md:NameIDFormat/text()" |> transform_by(&IDPMetadata.nameid_map/1),
        certificate: ~x"./md:IDPSSODescriptor/md:KeyDescriptor[@use='signing']/ds:KeyInfo/ds:X509Data/ds:X509Certificate/text()" |> transform_by(fn x -> :base64.decode(:erlang.list_to_binary(x)) end),
        tech: ~x"./md:ContactPerson[@contactType='technical']" |> transform_by(&Contact.decode/1),
        org: ~x"./md:Organization" |> transform_by(&Organization.decode/1)
      )
